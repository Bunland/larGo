...
extern JSValueRef DivF(JSContextRef context, JSObjectRef function, JSObjectRef thisObject, size_t argumentCount, JSValueRef arguments[], JSValueRef* exception);
*/
import "C"
import (
	"unsafe"
...
// DivF es la implementación de la función Div en C que será llamada desde JavaScript.
//
//export DivF
func DivF(context C.JSContextRef, function C.JSObjectRef, thisObject C.JSObjectRef, argumentCount C.size_t, arguments *C.JSValueRef, exception *C.JSValueRef) C.JSValueRef {
	if int(argumentCount) < 2 || arguments == nil {
		return C.JSValueMakeUndefined(context)
	}
...
// Div devuelve la función de callback de C para la función Div en JavaScript.
func Div() C.JSObjectCallAsFunctionCallback {
	return C.JSObjectCallAsFunctionCallback(C.DivF)
}
